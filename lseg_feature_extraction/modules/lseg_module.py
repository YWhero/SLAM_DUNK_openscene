...
class LSegModule(LSegmentationModule):
    def __init__(self, data_path, dataset, batch_size, base_lr, max_epochs, **kwargs):
        ...
        # =========================
        # PATCH: inference-only mode
        # =========================
        self.inference_only = bool(kwargs.get("inference_only", False)) or int(kwargs.get("max_epochs", 0)) == 0

        # inference-only에서는 학습 관련 속성들을 최소 정의
        if self.inference_only:
            # ADE20K는 보통 150 classes (encoding/ade20k 기준). 학습 안 하므로 값 자체는 사실상 의미 없음.
