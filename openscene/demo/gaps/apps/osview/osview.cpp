...
// 내가 추가한 곳 //
static RNVector *
ReadFloorPriorFile(const char *filename, int expected_length)
{
  ...
  if (!ReadNumpyFile(filename, &data_type, &data_size, &fortran_order, &width, &height, &depth, &array)) {
    fprintf(stderr, "Unable to read floor prior npy file %s\n", filename);
    return NULL;
  }

  // Flatten shape
  int n = 0;
  if (depth == 1 && height == 1) n = width;         // (N,)
  else if (depth == 1 && width == 1) n = height;    // (1,N) or (N,1) depending reader
  else if (depth == 1) n = width * height;          // fallback
  else {
    fprintf(stderr, "Unrecognized floor prior shape in %s (depth=%d)\n", filename, depth);
    delete [] array;
    return NULL;
  }

  if (expected_length > 0 && n != expected_length) {
    fprintf(stderr, "Floor prior length mismatch: prior=%d vs expected=%d (%s)\n", n, expected_length, filename);
    delete [] array;
    return NULL;
  }

  RNVector *prior = new RNVector(n);
  if (!prior) {
    fprintf(stderr, "Unable to allocate floor prior vector\n");
    delete [] array;
    return NULL;
  }

  // Copy values to float RNVector (expect uint8 or float; support common sizes)
  ...
  for (int i = 0; i < n; i++) {
    ...
    if (data_size == 1) {
      ...
      // half
      half_float::half *a = (half_float::half *) p;
      v = (float)(*a);
      p += 2;
    }
    else if (data_size == 4) {
      ...
      fprintf(stderr, "Unsupported floor prior dtype size=%d in %s\n", data_size, filename);
      delete prior;
      delete [] array;
      return NULL;
    }

    // Clamp to [0,1] 느낌으로 사용 (0/1 binary든 soft든 OK)
    if (v < 0) v = 0;
    if (v > 1) v = 1;
    (*prior)[i] = v;
  }
  ...
  for (int m = 0; m < point_features.NEntries(); m++) {
    ...
    if (query_features.NValues() == features->NColumns()) {
      ...
      for (int i = 0; i < features->NRows(); i++) {
        ...
        // 내가 추가한 곳 //
        // Apply floor prior ONLY when query is "floor"
        if (is_floor_query && (m < mesh_floor_priors.NEntries()) && mesh_floor_priors[m]) {
          RNVector *prior = mesh_floor_priors[m];
          if (i < prior->NValues()) {
...
main(int argc, char **argv)
{
  ...
  // 내가 추가한 곳 //
  // Read floor prior(s) if provided (expects one mesh for demo usage)
  if (input_floor_prior_filename) {
    if (meshes.NEntries() > 0) {
      R3Mesh *mesh0 = meshes.Kth(0);
      RNVector *prior0 = ReadFloorPriorFile(input_floor_prior_filename, mesh0->NVertices());
      if (prior0) {
        // ensure array size
        while (mesh_floor_priors.NEntries() < meshes.NEntries()) mesh_floor_priors.Insert(NULL);
        mesh_floor_priors[0] = prior0;
        printf("Loaded floor prior: %s (N=%d) for mesh[0]\n", input_floor_prior_filename, prior0->NValues());
      }
      else {
        printf("Warning: failed to load floor prior, continuing without it.\n");
      }
    }
  }
  // 내가 추가한 곳 //
